        ...
        try:
            ...
            if result.returncode == 0 and ocr_os.path.exists(tmp_out_path):
                with open(tmp_out_path, 'rb') as f:
                    pdf_bytes = f.read()
                print(f"✅ OCR text layer added! New size: {len(pdf_bytes)} bytes")
            else:
                print(f"⚠️ OCR failed (exit code {result.returncode}): {result.stderr}")
            ...
            print(f"⚠️ OCR text layer failed: {ocr_err}")
